...
void AGeometryHubActor::DoActorSpawn1() {
	// Получаем указатель на Мир Игры
		// Каждый Актор имеет функцию GetWorld 
		// который возвращает указатель на Глобальный объект мира игры
	UWorld* World = GetWorld();
	if (World) {
		for (int32 i = 0; i < 10; ++i) {
			// Класс FTransform - имеет множество различных конструкторов
			const FTransform GeometryTransform = FTransform(FRotator::ZeroRotator, FVector(0.0f, 300.0f * i, 300.0f));
			// мы сразу имеем указатель на объект нашего типа ABaseGeometryActor.
